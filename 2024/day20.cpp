...
int main()
{
    ...
    for (int i = 0; i < n; i++)
    {
        for (int j = 0; j < m; j++)
        {
            ...
            int disToSt = timeNormal - timeToEnd[i][j];
            int maxTime = 20;
            for (int endi = max(0, i - maxTime); endi < min(n, i + maxTime+1); endi++)
            {
                for (int endj = max(0, j - maxTime); endj < min(m, j + maxTime+1); endj++)
                {
                    int dis = abs(endi - i) + abs(endj - j);
                    if(dis > maxTime) continue;
                    if (map[endi][endj] != '#' && timeNormal - (disToSt + timeToEnd[endi][endj] + dis) >= 100)
                    {
                        ans[i][j].insert({endi, endj});
                    }
                }
            }
        }
